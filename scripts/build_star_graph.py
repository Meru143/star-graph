...
def build_graph(repos, enriched):
    ...
    for repo in repos:
        ...
        explicit = repo.get('topics', [])
        inferred = enriched.get(full_name, {}).get('inferred_topics', [])
        all_topics = list(set(explicit + inferred))
...
def export_graph(G):
    # Graphology JSON - supports lists
    ...
    # GraphML - convert all values to strings, skip None
    G_copy = nx.Graph()
    for node, data in G.nodes(data=True):
        gml_data = {}
